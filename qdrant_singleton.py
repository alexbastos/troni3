...
class QdrantSingleton:
    ...
    def search(self, categoria: str, vector: list, limit: int = 10):
        ...
        # 1. Tenta a API Moderna (qdrant-client >= 1.7)
        #    query_points espera o vetor puro + using para especificar o named vector
        ...
        # 2. Tenta a API Clássica (Se o seu pacote for mais antigo)
        elif hasattr(self.client, "search"):
            return self.client.search(
                collection_name=col_name,
                query_vector=NamedVector(name="text-dense", vector=vector), 
                limit=limit,
                with_payload=True
            )
            
        # 3. Tratamento de Erro Amigável para Shadowing de Arquivo
        else:
            raise AttributeError(
                f"O objeto instanciado em 'self.client' ({type(self.client)}) não possui "
                f"os métodos 'query_points' ou 'search'. Verifique se não existe nenhum arquivo "
                f"chamado 'qdrant_client.py' ou pasta de mesmo nome no seu diretório local "
                f"causando conflito de importação."
            )
