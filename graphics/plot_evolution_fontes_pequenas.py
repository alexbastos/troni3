...
def main():
    ...
    for cat in cat_names:
        ...
        # Como o backup do DD_DLS pode conter amostras incompletas devido à conexão,
        # vamos usar o cálculo real dos dados existentes.
    ...
    if lat_antes_rag and lat_depois_rag:
        ...
        # Adicionar médias textuais para complementar o boxplot
        mean_antes = np.mean(lat_antes_rag)
        mean_depois = np.mean(lat_depois_rag)
        ax.text(0, mean_antes, f"Média: {mean_antes:.2f}s", ha='center', va='bottom', color='black', fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.2'))
        ax.text(1, mean_depois, f"Média: {mean_depois:.2f}s", ha='center', va='bottom', color='black', fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.2'))
